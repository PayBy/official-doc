...
export async function orderCreation(data, callback, env) {
  ...
  let host = 'https://uat-checkout.test2pay.com'
  if (env === 'development') {
...
export async function queryOrder(data, callback, env) {
  try {
    const { merchantOrderNo } = data
    const body = {
      signature: true,
      requestTime: moment().valueOf(),
      bizContent: {
    ...
    const sign = await encrypt(body)
    const response = await fetch(host + '/api/sgs/acquire2/getOrder', {
      headers: {
        'Content-Language': 'en',
        'Partner-Id': '200000042613',
        sign,
