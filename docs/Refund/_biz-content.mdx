...
- **sharingParamList** <font color = ' #7d8793'>List</font>
  ...
  - **sharingIdentity**   <font color = ' #7d8793'>String</font>    <font color = '#f19938'>Required</font>
    ...
    After selecting the identity type, what value should be passed. The parameter needs to be encrypted when passed, and RSA algorithm is recommended.
  ...
  - **withholdAndRemitFee**   <font color = ' #7d8793'>Boolean</font>    
    ...
    If in the original payment order, **withholdAndRemitFee** is `True` for this sharing account, then in the refund order, **withholdAndRemitFee** should also be `True` for this sharing account. The actual amount deducted from this sharing account is [**sharingAmount** -PayBy Transaction Fee refunded]. The transaction fee will be refunded in proportion to the amount of the refunded order/original payment order.
  
    Same rule for the case **withholdAndRemitFee** is `False`.
