...
import ResponseHead from '../3_Refund/_response-head.mdx'
import TransferToBankOrder from './_transfer-to-bank-order.mdx'

# Transfer to Bank

> **Note:** Before using this API, please use `Get fund out ability` API to retrieve the supported payout methods and which parameters need to be passed for each payout method. If the currency of received amount is not AED, please use the `Rate Calculator` API to get the latest exchange rate.
