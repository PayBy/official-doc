---
sidebar_position: 7
toc_max_heading_level: 6
---

import ResponseHead from '../_request-header.mdx'

# Get Cashier URL Information

## Overview

This interface returns information about whether a QR code has been scanned by the user and provides details about the scanner (payer).

## API URL

- **Testing environment**: `https://uat.test2pay.com/sgs/api/acquire2/getCashierUrlInfo`
- **Production environment**: `https://api.payby.com/sgs/api/acquire2/getCashierUrlInfo`

## Request

### HTTP Headers

<details>
  <summary>Attributes</summary>
  <RequestHeader/>
</details>

### HTTP Body



### **Request Object**

- **Request Time**  
	- **Parameter Name**: `requestTime`  
	- **Required**: Yes  
	- **Type**: `Timestamp(3)`  
	- **Example**: `1581493898000`  
	- **Description**: Request timestamp in milliseconds.

- **Business Content**  
	- **Parameter Name**: `bizContent`  
	- **Required**: Yes  
	- **Type**: `GetCashierUrlInfoRequest`
		- **Token URL**  
			- **Parameter Name**: `tokenUrl`  
			- **Required**: Yes  
			- **Type**: `String`  
			- **Example**: `https://checkout.payby.com/pay-page/main?BIZ_TYPE=202&ft=cd624549f4894d78b0d368e48bb27aae&t=1654519114067`  
			- **Description**: The cashier URL used to initiate or query the payment process.
	- **Example**: *(none provided)*  
	- **Description**: Business request content containing the necessary parameters for generating the cashier URL.


### Request Sample

#### HTTP Headers

```json
{
    "Content-Language": "en",
    "Content-Type": "application/json",
    "Partner-Id": "200000018128",
    "sign": ""
}
```

#### HTTP Body

```json
{
  "requestTime": 1581406091642,
  "bizContent": {
    "tokenUrl": "https://checkout.payby.com/pay-page/main?BIZ_TYPE=202&ft=cd624549f4894d78b0d368e48bb27aae&t=1654519114067"
  }
}
```

## Response

### HTTP Headers

- **Signature**  
	- **Parameter Name**: `Sign`  
	- **Required**: Yes  
	- **Type**: `String`  
	- **Example**: *(none provided)*  
	- **Description**: Response signature used to verify the integrity and authenticity of the response data.


### HTTP Body

>  **Note:** The `body` field in the HTTP response is returned only when `applyStatus` is `SUCCESS` and `code` is `0`.

- **Response Header**  
	- **Parameter Name**: `head`  
	- **Required**: Yes  
	- **Type**: `ResponseHeader`  
	- **Example**: *(none provided)*  
	- **Description**: Response header information containing metadata such as status codes and messages.

- **Response Body**  
	- **Parameter Name**: `body`  
	- **Required**: No  
	- **Type**: `GetCashierUrlInfoResponse`  
		- **Payer Information**  
			- **Parameter Name**: `payer`  
				- **Mobile Number Mask**  
					- **Parameter Name**: `mobileNumberMask`  
					- **Required**: No  
					- **Type**: `String`  
					- **Example**: `+971-58*****92`  
					- **Description**: Masked mobile number of the payer, used to partially conceal the full number for privacy and security.
			- **Required**: Yes  
			- **Type**: `Payer`  
			- **Example**: *(none provided)*  
			- **Description**: Information about the person making the payment, including identity and payment details.
	- **Example**: *(none provided)*  
	- **Description**: Response body content containing the main data returned by the API.

### Response Sample

#### HTTP Headers

```json
{
	"sign": ""
}
```

#### HTTP Body

```json
{
    "body":{
        "payer":{
            "mobileNumberMask":"+971-58*****92"
        }
    },
    "head":{
        "applyStatus":"SUCCESS",
        "code":"0",
        "msg":"SUCCESS",
        "success":true,
        "traceCode":"975929"
    }
}
```

## Response Codes

| Code  | Message               | Cause                           | Workaround                    |
| ----- | --------------------- | ------------------------------- | ----------------------------- |
| 0     | SUCCESS               | Request completed successfully  | -                             |
| 400   | INVALID_PARAMETER     | Invalid parameter provided      | Adjust request parameters     |
| 400   | REQUESTTIME_TOO_EARLY | Request time is too early       | Adjust request time           |
| 400   | REQUESTTIME_TOO_LATER | Request time is too late        | Adjust request time           |
| 402   | RATE_LIMIT_REJECT     | Request rate limit exceeded     | Reduce request frequency      |
| 403   | UNAUTHORIZED          | API not authorized              | Contact PayBy                 |
| 404   | SERVICE_NOT_AVAILABLE | API service unavailable         | Contact PayBy                 |
| 500   | SYSTEM_ERROR          | System error occurred           | Contact PayBy and retry later |
| 504   | SERVICE_TIMEOUT       | Service timeout                 | Retry later                   |
| 601   | RISK_FAIL             | Risk control validation failed  | Adjust business logic         |
| 62082 | TOKEN_URL_NOT_EXIST   | Token URL is invalid or expired | Adjust request parameters     |
